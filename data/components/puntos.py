...
class Puntos(object):
    """Puntos that appear, float up, and disappear"""
    def __init__(self, x, y, puntos, flag_pole=False):
        self.x = x
        self.y = y
        if flag_pole:
            self.y_vel = -4
        ...
        self.puntos_string = str(puntos)
        self.create_digit_list()
        self.flag_pole_puntos = flag_pole
    ...
    def create_digit_list(self):
        """Creates the group of images based on puntos received"""
        self.digit_list = []
        self.digit_group = pg.sprite.Group()

        for digit in self.puntos_string:
    ...
    def update(self, puntos_list, level_info):
        """Updates puntos movement"""
        for number in self.digit_list:
            number.rect.y += self.y_vel

        if puntos_list:
            self.check_to_delete_floating_puntos(puntos_list, level_info)

        if self.flag_pole_puntos:
            if self.digit_list[0].rect.y <= 120:
                self.y_vel = 0


    def draw(self, screen):
        """Draws puntos numbers onto screen"""
        for digit in self.digit_list:
            screen.blit(digit.image, digit.rect)


    def check_to_delete_floating_puntos(self, puntos_list, level_info):
        """Check if puntos need to be deleted"""
        for i, puntos in enumerate(puntos_list):
            if int(puntos.puntos_string) == 1000:
                if (puntos.y - puntos.digit_list[0].rect.y) > 130:
                    puntos_list.pop(i)

            else:
                if (puntos.y - puntos.digit_list[0].rect.y) > 75:
                    puntos_list.pop(i)
